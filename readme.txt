...
Single Sign-On ( SSO ) allows users to login into any site/application using the credentials of another app/site.
...
*	**Advanced Attribute Mapping** : OAuth Login (Single Sign On) provides an Attribute Mapping feature to map WordPress user profile attributes like username, firstname, lastname, email and profile picture. Manage username & email with data provided.
...
*	**Multisite Support** : WordPress Multisite allows you to create multiple subdomains / subdirectories within a single instance of WordPress. OAuth Login(Single Sign On) has the unique ability to support multiple sites (multisite) under one account. You have to configure the OAuth / OpenID Connect SSO plugin just once for all your sites in the network.
...
*	**Single Sign On Login Reports** : OAuth Login (Single Sign On) creates user login and registration reports based on the application used.
*	**Enable/Disable WordPress Default Login** : OAuth Login allows you to disable the default WordPress login form.
*	**Multisite Support** : WordPress Multisite allows you to create multiple subdomains / subdirectories within a single instance of WordPress. OAuth Login(Single Sign On) have unique ability to support multiple sites (multisite) under one account. You have to configure the OAuth / OpenID Connect SSO plugin just once for all your sites in the network.

= ALL INCLUSIVE VERSION FEATURES =

*All-in-one SSO plan with the support of add-ons / Third Party Plugin compatibility
...
*	**LearnDash Integrator** - LearnDash Integration will map the SSO users to LearnDash groups as per the group attributes sent by your Identity Provider.
*	**Login Form Add-On** - This add-on provides Login form for OAuth/OpenID login instead of only a button. It relies on OAuth/OpenID plugin to have Password Grant configured. It can be customized using custom CSS and JS.
...
*	**Attribute Based Redirection** - Attribute Based Redirection add-on can be used to restrict and redirect users to different URLs based on OAuth/OpenID attributes.
...
*	**Membership Level based Login Redirection** - This add-on allows redirecting users to custom pages based on users' membership levels after Single Sign On. Checks for the user's membership level during every login, so any update on the membership level doesn't affect redirection.
