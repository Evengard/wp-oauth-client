 ...
 */
function mo_oauth_client_setup_apps(){
	...
	echo '<center><h4>OAuth / OpenID Connect Providers</h4></center>
		...
		<div class="_3HxD">
			<div class="q8lC">
				...
				<input type="text" name="mo_oauth_search" autofocus=true autocomplete=false value id="mo_oauth_client_default_apps_search" onkeyup="mo_oauth_client_default_apps_input_filter()" placeholder="'. __("Search Your Provider","miniorange-login-with-eve-online-google-facebook") . '">
			</div>
		</div>
	<!-- app list -->
	...
	<div id="mo_oauth_client_searchable_apps">
		<ui id="mo_oauth_client_default_apps" class="mo-flex-container mo-wrap">';
			foreach($defaultappsjson as $appId => $application) {
				...
				echo '<li data-appid="'.esc_attr($appId).'" class="mo-flex-item"><a ' . ( "cognito" === $appId ? "id=vip-default-app" : "" ) . ' href="#"><img class="mo_oauth_client_default_app_icon" src="'. plugins_url( '/partials/apps/images/'.$application->image, dirname(dirname(dirname(__FILE__ )))).'"><br><p>'.esc_attr($application->label).'</p></a><input type="hidden" value="'.esc_html(json_encode($application)).'"></li>';				
			}
	...
	echo '<li class="mo-flex-item hidden-flex-item"></li><li class="mo-flex-item hidden-flex-item"></li><li class="mo-flex-item hidden-flex-item"></li><li class="mo-flex-item hidden-flex-item"></li></ui></div></form><script>
			jQuery("#mo_oauth_client_default_apps li").click(function(){
				...
				jQuery("#moauth_show_desc").html("This will displayed on SSO login button as <b>\"Login with " +appId +"\"</b>");
