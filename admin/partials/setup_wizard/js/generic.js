...
function mo_oauth_client_default_apps_input_filter() {
    ...
    for (i = 0; i < li.length; i++) {
        ...
        if(counter>=li.length) {
            document.getElementById("mo_oauth_client_search_res").innerHTML = "<p class='lead muted mo_premium_features_notice'>Oops! It looks like your OAuth provider is not listed below, you can select custom provider to configure the plugin. Please send us a query using support if you need any help in configuration.</p>";
            li[0].style.display = "";
            li[1].style.display = "";
            li[2].style.display = "";
        } else {
 ...
 */
function mo_oauth_auto_fill_form(){
   
    if (null != app && (null != app.mo_oauth_appId || undefined != app.mo_oauth_appId )) {
         mo_oauth_get_step(app.mo_oauth_step);
        ...
        mo_oauth_get_summary();       
    }
    else{
         mo_oauth_get_step(1);
    }   
}
function mo_oauth_update_display_name_description(){
 ...
 */
function mo_oauth_get_data(option,action){
 ...
 */
function mo_oauth_get_step(step){
 ...
 */
function mo_outh_input_validation(){
    var mo_auth_response = 'success';
     li =jQuery(".mo-summary-data");
     for (i = 0; i < li.length; i++) {
        if (2 === i) {
            var mo_auth_value = jQuery("#client_id_summary").val();
        }
        else if(3===i){
            var mo_auth_value = jQuery("#client_secret_summary").val();    
        }
        else{
 ...
 */
function showClientSecret(clientSecretId,show_buttonId){
 ...
 * SSO test is invoked
 */
function testConfiguration(site_url){
