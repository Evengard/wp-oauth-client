...
class Mo_OAuth_Client_Setup_Wizard{
	...
	public function page(){
		echo '<head>
		    <meta charset="utf-8">
		    <!-- load dependencies -->';
		    wp_enqueue_style( 'mo_oauth_setup_wizard_style',plugin_dir_url( __FILE__ ) . '/css/generic.css',array(),MO_OAUTH_SETUP_WIZARD_VERSION );
			wp_print_styles('mo_oauth_setup_wizard_style');
			wp_enqueue_style( 'mo_oauth_setup_wizard_steps_style',plugin_dir_url( __FILE__ ) . '/css/multi-step.css',array(),MO_OAUTH_SETUP_WIZARD_VERSION );
			wp_print_styles('mo_oauth_setup_wizard_steps_style');
			wp_enqueue_style( 'mo_oauth_setup_wizard_font_style',plugin_dir_url( dirname(dirname(dirname(__FILE__ )))) . '/css/font-awesome.css',array(),MO_OAUTH_SETUP_WIZARD_VERSION);
			wp_print_scripts('jquery');
			wp_print_scripts('jquery-ui-tooltip');
			wp_print_styles('mo_oauth_setup_wizard_font_style');
			wp_print_styles('dashicons');
			wp_enqueue_script( 'mo_oauth_setup_wizard_script', plugin_dir_url( __FILE__ ) . '/js/generic.js',array(),MO_OAUTH_SETUP_WIZARD_VERSION );
			wp_localize_script( 'mo_oauth_setup_wizard_script', 'mo_oauth_ajax_object',
            array( 'ajax_url' => admin_url( 'admin-ajax.php' ), 'app' => get_option('mo_oauth_setup_wizard_app') ) );
					...
					<div class="mo-multistep-root">
						<span class="mo-multilabel-root">
							...
							<span class="mo-muisteplabel-label-description">
								Copy & paste callback URL at OAuth Provider end
							</span>
						</span>
					</div>
					<div class="mo-multistep-root">
						...
						<span class="mo-multilabel-root">
							<span class="mo-multistep-icon-container">
								<div class="mo-icon-style icon-step3">
									<svg class="mo-multistep-svgicon" focusable="false" viewBox="0 0 24 24" aria-hidden="true">
							...
							<span class="mo-muisteplabel-labelcontainer mo-label-step3">
								Discovery
							</span>
							<span class="mo-muisteplabel-label-description">
								Add disovery URL/endpoints to connect with provider
							</span>
						</span>
					</div>
					<div class="mo-multistep-root">
						<div class="mo-muilti-step-connector">
							<span class="mo-muilti-step-connector-span mo-muilti-step-connector4">
							</span>
						</div>					
						<span class="mo-multilabel-root">
							<span class="mo-multistep-icon-container">
							...
							<span class="mo-muisteplabel-label-description">
								Add client Id & secret for verification at provider end
							</span>
						</span>
					</div>
				</div>
		    ...
		    <div class="aui-page-panel">
		    	<div class="aui-page-panel-inner">
		    		<section class="aui-page-panel-content">
				    	<div id="step1" style="display:none">';			    	
				   			mo_oauth_client_setup_apps();
				echo 	'</div>
						<div id="step2" style="display:none">';
				    		mo_oauth_client_setup_callback();
				echo	'</div>
						<div id="step3" style="display:none">';
							mo_oauth_client_setup_endpoints();
				echo	'</div>
						<div id="step4" style="display:none">';
							mo_oauth_client_setup_client();
				echo	'</div>
						<div id="step5" style="display:none">';
							mo_oauth_client_summary();
				echo	'</div>		
				...
				echo	'</div>			
				        ...
				        <div class="mo-button__footer">
				            ...
				            <div>
				                <input type="submit" name="later" id="mo-link-draft" class="mo-button--link mo-oauth-save-draft" href="." value="Save draft">
			...
			<script>
				...
				console.log("#"+target);
					if("mo-btn-next" == target){
						var data= mo_oauth_get_data("save_draft","next");
					}
					if("mo-btn-back" == target){
						var data= mo_oauth_get_data("save_draft","back");
				    	
					}
					if("mo-link-draft" == target){
						var data= mo_oauth_get_data("save_draft","draft");
					}					
					if("mo-btn-finish" == target){

						var data= mo_oauth_get_data("save_app","finish");
					}										
					if("mo-btn-close" == target){

						var data= mo_oauth_get_data("save_draft","close");
					}										
					if("mo-btn-skip" == target){
						var data= mo_oauth_get_data("save_draft","skip");
					}
					if("mo-btn-next" == target || "mo-btn-back" == target){
						...
						var mo_response = mo_outh_input_validation();
						if(mo_response == "success"){
							jQuery.post(mo_oauth_ajax_object.ajax_url, data, function(response){
							if("mo-btn-finish" == target){
								var site_url = "'.esc_url_raw(site_url()).'";
								testConfiguration(site_url);
								mo_oauth_test_ajax_count = 0;
								mo_oauth_get_test_result();
								mo_oauth_trace_test_progress = setInterval(mo_oauth_get_test_result, 5000);
							}
				...
				jQuery("#mo-btn-test-finish").click(function(){
					var data = {
						"action": "mo_outh_ajax",
